...
| **LT gross AEP — methods** | **Method A:** long-term Weibull × warranted curve (integral). **Method B:** production regression (daily E=c·v³ / monthly E=a+b·WS). **UL/OEPR Step-1 (recommended, `lt_primary_method: "ul"`):** UL-style monthly production regression — monthly gross energy normalised to 30-day months (Eq. 3), density-corrected reference wind speeds (Eq. 4), OLS Prod_norm = a·WS_ref + b (Eq. 5) applied to the full LT record and de-normalised to calendar months (Eq. 6); supports excluded months and a future-loss stack (`excluded_months`, `future_losses`) |
